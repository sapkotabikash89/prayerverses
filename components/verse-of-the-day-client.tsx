...
import { Facebook, Twitter, Mail, Link as LinkIcon, Share2, Download } from 'lucide-react'
...
const BACKGROUNDS = [
  {
    ...
    exportBg: '#fff7ed', // amber-50 equivalent for export
  },
  {
    id: 'forest',
    label: 'Forest',
...
interface VerseOfTheDayClientProps {
  verse: {
    text: string
    ref: string
  }
...
export function VerseOfTheDayClient({ verse, date }: VerseOfTheDayClientProps) {
  ...
  const downloadImage = async () => {
    ...
    try {
      ...
      link.download = `verse-of-the-day-${date}.png`
      link.href = dataUrl
      link.click()
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="mt-8 flex flex-wrap justify-center gap-4">
        ...
        <Button variant="outline" size="icon" className="rounded-none" onClick={() => window.open(`mailto:?subject=Bible Verse of the Day&body=${encodeURIComponent(shareText + '\n' + shareUrl)}`, '_blank')}>
